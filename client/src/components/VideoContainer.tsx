  ...
  return (
    <Wrapper displayingVideo={displayingVideo}>
        <VideoWrapper>
          ...
          {
              peers.map((peer) => <PeerCell key={peer.peerID} peer={peer.peer} />)
          }
        </VideoWrapper>
        <OperationRow>
            <BackToChatBtn variant='contained' onClick={backToChat}>Back to chat</BackToChatBtn>
            <VideoOperationRow>
                <MuteSelfBtn onClick={toggleSelfMute}>
                    { muteSelf ? <AiOutlineAudio /> : <AiOutlineAudioMuted /> }
                </MuteSelfBtn>
                <CloseCamBtn onClick={toggleCloseCam}>
                    { closeCam ? <FiCameraOff /> : <FiCamera /> }
                </CloseCamBtn>
                <MuteOthersBtn onClick={toggleOthersMute}>
                    { muteOthers ? <BsFillVolumeMuteFill /> : <BsFillVolumeUpFill /> }
                </MuteOthersBtn>
            </VideoOperationRow>
            <LeaveCallBtn variant='contained' onClick={LeaveVideoChat}>Leave</LeaveCallBtn>
        </OperationRow>
    </Wrapper>
  )
}
